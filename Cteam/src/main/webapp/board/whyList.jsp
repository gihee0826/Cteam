...
<%
	...
	String totalSql  = "select count(*) as cnt ";
	   ...
	   totalSql +=" and b.bcategory = '궁금하다냥'";
...
<head>
    ...
    <script>
		...
		function go(bno,searchType,searchValue,nowPageStr){
			...
			if(login != 'null'){
				location.href='whyDetail.jsp?bno='+bno+'&searchType='+searchType+'&searchValue='+searchValue+'&nowPageStr='+nowPageStr;
				//항상 구문정확히 적기
			}else{
...
<body>
    <div class="mainBox">
        ...
        <section >
        <form action="whyList.jsp" method="get">
            <div class="middleBar">
            ...
            <div class="body">
				...
				%>
					<a href="whyList.jsp?nowPage=<%=paging.getStartPage()-1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">&lt;</a>
				<%
					}
					...
					for(int i=paging.getStartPage();i<=paging.getEndPage(); i++){
						if(i == paging.getNowPage()){
							...
							%>
								<a href="whyList.jsp?nowPage=<%=i%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>"><%=i%></a>
							<%
						}
				...
				%>
						<a href="whyList.jsp?nowPage=<%=paging.getEndPage()+1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">&gt;</a>
				<%
