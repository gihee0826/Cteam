...
<head>
    ...
    <script>
		...
		function go(bno,searchType,searchValue,nowPageStr){
			...
			if(login != 'null'){
				location.href='helloDetail.jsp?bno='+bno+'&searchType='+searchType+'&searchValue='+searchValue+'&nowPageStr='+nowPageStr;
				//항상 구문정확히 적기
			}else{
...
<body>
    <div class="mainBox">
        ...
        <section >
        <form action="helloList.jsp" method="get">
            <div class="middleBar">
            ...
            <div class="body">
				...
				%>
					<a href="helloList.jsp?nowPage=<%=paging.getStartPage()-1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">&lt;</a>
				<%
					}
					...
					for(int i=paging.getStartPage();i<=paging.getEndPage(); i++){
						if(i == paging.getNowPage()){
							...
							%>
								<a href="helloList.jsp?nowPage=<%=i%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>"><%=i%></a>
							<%
						}
				...
				%>
						<a href="helloList.jsp?nowPage=<%=paging.getEndPage()+1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">&gt;</a>
				<%
