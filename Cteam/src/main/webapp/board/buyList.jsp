...
<head>
    ...
    <script>
		...
		function go(bno,searchType,searchValue,nowPageStr){
			...
			if(login != 'null'){
				location.href='buyDetail.jsp?bno='+bno+'&searchType='+searchType+'&searchValue='+searchValue+'&nowPageStr='+nowPageStr;
				//항상 구문정확히 적기
			}else{
    ...
    <div class="mainBox">
        ...
        <form action="buyList.jsp" method="get">
            ...
            <div class="body">
				...
				%>
					<a href="buyList.jsp?nowPage=<%=paging.getStartPage()-1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">&lt;</a>
				<%
					}
					...
					for(int i=paging.getStartPage();i<=paging.getEndPage(); i++){
						if(i == paging.getNowPage()){
							...
							%>
								<a href="buyList.jsp?nowPage=<%=i%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>"><%=i%></a>
							<%
						}
				...
				%>
						<a href="buyList.jsp?nowPage=<%=paging.getEndPage()+1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">&gt;</a>
				<%
